...
## 1.a) Format fasta files - Vd


### for V. dahliae isolate JR2
```bash
  Taxon_code=Vd
  Fasta_file=$(ls $ProjDir/../clocks/public_genomes/JR2/Verticillium_dahliaejr2.GCA_000400815.2.pep.all.fa)
  Id_field=1
  orthomclAdjustFasta $Taxon_code $Fasta_file $Id_field
  mv "$Taxon_code".fasta $WorkDir/formatted/"$Taxon_code".fasta
```

### for N. crassa isolate
```bash
  Taxon_code=Nc
  Fasta_file=$(ls $ProjDir/../../neurospora_crassa/Neurospora_crassa.NC12.pep.all.fa)
  Id_field=1
  orthomclAdjustFasta $Taxon_code $Fasta_file $Id_field
  mv "$Taxon_code".fasta $WorkDir/formatted/"$Taxon_code".fasta
```
